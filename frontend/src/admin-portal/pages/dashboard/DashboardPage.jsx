// src/admin-portal/pages/dashboard/DashboardPage.jsx
// Built last, on top of surfaces that already exercise these endpoints. The
// four KPI cards are exactly and only backend/src/modules/analytics
// /analytics.service.ts's dashboardStats() fields - no invented trend arrows.
// The one exception is Overdue's sparkline, which is real: it's the same
// `overdue` field already loaded for the Overdue-rate chart below, not a
// fabricated series. The other three KPIs have no daily breakdown anywhere
// in the backend, so they render without a trend pill or sparkline rather
// than inventing one - see KpiCard.jsx's own comment on this.
...
import { ActiveLoansIcon, OverduesIcon, FinesIcon, ItemsAddedIcon } from '../../components/common/Icons';
...
export function DashboardPage() {
  ...
  // The Overdue KPI's sparkline reuses the SAME `overdue` series already
  // fetched for the Overdue-rate chart below (not a second request, not a
  // fabricated one) - the last 10 days' worth is enough to read as a trend
  // without dwarfing the card.
